...
contains
  ...
  subroutine CalcIrrigationNeeded(this, bounds, num_exposedvegp, filter_exposedvegp, &
       elai, t_soisno, eff_porosity, h2osoi_liq, depth_h2osfc, volr, rof_prognostic)
    !
    ! !DESCRIPTION:
    ...
    real(r8), intent(in) :: depth_h2osfc(bounds%begc: )
    ...
    do fc = 1, check_for_irrig_col_filter%num
       c = check_for_irrig_col_filter%indices(fc)
       !if (depth_h2osfc(c) < 20.0) then
       !   deficit_pool(c) = 100.0 - depth_h2osfc(c)
       !else
  ...
  function PointNeedsCheckForIrrig(this, pft_type, elai, londeg) &
    ...
    !
    ! !USES:
    use clm_time_manager, only : get_local_time, get_curr_calday
    use pftconMod       , only : pftcon
    !
    ! !ARGUMENTS:
